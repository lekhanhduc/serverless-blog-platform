    ...
    useEffect(() => {
        if (user) {
            userApi.getMe().then(({ data }) => {
                ...
                setProfile({ username: data.username, email: data.email, role: data.role });
            }).catch(() => {});
        } else {
            setAvatarUrl(null);
            setProfile(null);
                            ...
                            <div className="flex items-center gap-3 px-3 py-2 mb-2 bg-gray-50 rounded-lg">
                                {avatarUrl ? (
                                    ...
                                    <div className="w-10 h-10 bg-primary rounded-full flex items-center justify-center text-white font-semibold">
                                        {profile?.username?.[0]?.toUpperCase() || user.name?.[0]?.toUpperCase() || 'U'}
                                    </div>
                                )}
                                <div className="flex-1 min-w-0">
                                    <p className="font-medium text-gray-900 text-sm truncate">{profile?.username || user.name || 'User'}</p>
                                    <p className="text-xs text-gray-500 truncate">{profile?.email || user.signInDetails?.loginId}</p>
                                </div>
                            </div>
                            <button
                                onClick={() => { logout(); go('/'); }}
